...
pthread_mutex_t escribirListaUMV;
pthread_mutex_t semaforoTipoProceso;
...
segmento->posMemoria=(int)malloc(CANT_MEMORIA_TOTAL); //Creo el "gran malloc" (GM)
segmento->segmentoReservado=1;
segmento->size=atoi(MALLOC);

log_info(ptrLog,"Creado el bloque de memoria en donde se guarda el GM");
