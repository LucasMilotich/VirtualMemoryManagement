...
char* solicitarBytes(t_solicitarBytes *solicitarBytes, int* procesoActivo) {
	...
	if ((solicitarBytes->tamanio + solicitarBytes->offset)> segmentoEncontrado->size) {
			char* buffer = malloc(sizeof(int));
			memcpy(buffer, &respuestaEnCasoDeError, sizeof(int));
			sleep(tiempoRetardo);
			log_error(ptrLog, "El tamanio + offset es mayor al size del segmento");
			pthread_mutex_unlock(&escribirListaUMV);
			return buffer;
	...
	void* c;
	c = (segmentoEncontrado->posMemoria) + (solicitarBytes->offset);
...
//	}else{
	...
	void* c;
	c = (segmentoEncontrado->posMemoria) + (enviarBytes->offset);
